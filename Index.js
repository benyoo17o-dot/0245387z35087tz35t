...
const CONFIG = {
DISCORD_TOKEN: process.env.DISCORD_TOKEN || ‘ MTQyODEyNzk4MTgyOTc1MDgwNQ.GArHew.gOE_XYLqFkGVvvt9EkhS5nXvXecl8Esx8SQifY’,
DASHBOARD_PORT: 5000,
};
...
const commands = [
{ name: ‘ping’, description: ‘Zeigt den Bot Ping’ },
{
name: ‘embed’,
description: ‘Sendet ein Custom Embed’,
options: [
{ name: ‘titel’, type: 3, description: ‘Titel des Embeds’, required: true },
{ name: ‘beschreibung’, type: 3, description: ‘Beschreibung des Embeds’, required: true },
],
},
{
name: ‘kick’,
...
options: [
{ name: ‘nutzer’, type: 6, description: ‘Der zu kickende Nutzer’, required: true },
{ name: ‘grund’, type: 3, description: ‘Grund fuer den Kick’, required: false },
],
},
{
name: ‘ban’,
...
options: [
{ name: ‘nutzer’, type: 6, description: ‘Der zu bannende Nutzer’, required: true },
{ name: ‘grund’, type: 3, description: ‘Grund fuer den Ban’, required: false },
],
},
{
name: ‘mute’,
...
options: [
{ name: ‘nutzer’, type: 6, description: ‘Der zu mutende Nutzer’, required: true },
{ name: ‘dauer’, type: 4, description: ‘Dauer in Minuten’, required: false },
],
},
{ name: ‘lock’, description: ‘Sperrt einen Channel’ },
{ name: ‘unlock’, description: ‘Entsperrt einen Channel’ },
{
name: ‘clear’,
description: ‘Loescht Nachrichten’,
options: [
{ name: ‘anzahl’, type: 4, description: ‘Anzahl der Nachrichten zum Loeschen’, required: true },
],
},
];
...
if (commandName === 'clear') {
  ...
  if (anzahl > 100) return interaction.reply({ content: 'Maximal 100 Nachrichten!', ephemeral: true });
  try {
    await interaction.channel.bulkDelete(anzahl);
    await interaction.reply({ content: `${anzahl} Nachrichten geloescht`, ephemeral: true });
  } catch (error) {
    await interaction.reply({ content: 'Konnte Nachrichten nicht loeschen!', ephemeral: true });
  }
}
```
...
const VALID_USERS = { ‘admin’: ‘rekii123’, ‘owner’: ‘owner2025’ };

app.post(’/api/login’, (req, res) => {
const { username, password } = req.body;
if (VALID_USERS[username] === password) {
res.json({ success: true, token: Buffer.from(`${username}:${password}`).toString(‘base64’), user: username });
} else {
res.status(401).json({ success: false, message: ‘Ungueltige Anmeldedaten’ });
}
});

function authMiddleware(req, res, next) {
const token = req.headers.authorization?.split(’ ‘)[1];
if (!token) return res.status(401).json({ message: ‘Keine Authentifizierung’ });
try {
...
app.listen(PORT, () => {
console.log(`Dashboard API laeuft auf http://localhost:${PORT}`);
});
